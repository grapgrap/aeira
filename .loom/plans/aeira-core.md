...
> 프리미티브 기반 쿼리 인터페이스 (0008). 소비자(loom/shulker)가 조합하는 저수준 프리미티브 세 개.

#### 작업 견적

- neighbors: 낮음 — adjacency list에서 직접 조회
- path: 보통 — BFS/DFS + 순환 방어 + 경로 수 상한
- graph: 낮음 — 전체 엣지 덤프
- CLI 명령 + 출력 포맷: 보통
...
- 의존: 5
- 병렬: neighbors와 graph는 독립적으로 구현 가능

#### 세부 작업 목록

- [x] shape: 소비자의 문서 탐색 패턴 분석 → 프리미티브 도출
- [x] neighbors 쿼리 — 1-hop 이웃 반환 (outgoing/incoming/both)
- [x] path 쿼리 — 두 노드 간 모든 경로 반환. 노드 재방문 금지(순환 방어), 경로 수 상한
- [x] graph 쿼리 — 전체 노드 + 엣지 목록
- [x] 출력 포맷 — LLM 친화적 텍스트 기본, --json 옵션
- [x] CLI 명령 스캐폴딩
- [x] 쿼리 테스트
